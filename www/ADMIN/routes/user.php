...
$app->post('/user/add', function () use ($app) {
    ...
    $result = AdminUser::create($app->request->post('name'), $app->request->post('mail'), $app->request->post('login'), $app->request->post('password'), $app->request->post('user_active') == 'on' ? 1 : 0, $app->request->post('user_admin') == 'on' ? 1 : 0);
