...
@section("breadcrumb", BreadcrumbHelper::renderAdmin(array(
    ...
    Lang::get("views/pages/navigation.admin.users.show.name")
)))

@section("master-head")
    <div class="row">
        <div class="small-12 columns">
            <h1>{{{ $user->name }}} <a href="{{ URL::route("admin.users.edit", array("id" => $user->id)) }}" class="button right" title="{{ Lang::get("views/pages/navigation.admin.users.edit.title") }}" role="button"><span class="glyphicons pencil"></span> {{ Lang::get("views/pages/navigation.admin.users.edit.name") }}</a></h1>
        </div>
        <!-- /small-12.columns -->
    </div>
    <!-- /row -->
