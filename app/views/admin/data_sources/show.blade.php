...
@section("breadcrumb", BreadcrumbHelper::renderAdmin(array(
    ...
    Lang::get("views/pages/navigation.admin.data_sources.show.name")
)))

@section("master-head")
    <div class="row">
        <div class="small-12 columns">
            <h1>{{{ $dataSource->name }}} <a href="{{ URL::route("admin.data-sources.edit", array("id" => $dataSource->id)) }}" class="button right" title="{{ Lang::get("views/pages/navigation.admin.data_sources.edit.title") }}" role="button"><span class="glyphicons pencil"></span> {{ Lang::get("views/pages/navigation.admin.data_sources.edit.name") }}</a></h1>
        </div>
        <!-- /small-12.columns -->
    </div>
    <!-- /row -->
