...
@section("breadcrumb", BreadcrumbHelper::renderAdmin(array(
    ...
    Lang::get("views.shared.navigation.admin.data_types.show.name")
)))

@section("master-head")
    <div class="row">
        <div class="small-12 columns">
            <h1>{{{ $dataType->label }}} <a href="{{ URL::route("admin.data-types.edit", array("id" => $dataType->id)) }}" class="button right" title="{{ Lang::get("views.shared.navigation.admin.data_types.edit.title") }}" role="button"><span class="glyphicons pencil"></span> {{ Lang::get("views.shared.navigation.admin.data_types.edit.name") }}</a></h1>
        </div>
        <!-- /small-12.columns -->
    </div>
    <!-- /row -->
