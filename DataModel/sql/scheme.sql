...
CREATE  TABLE IF NOT EXISTS `tools_registry`.`tool_has_platform` (
  ...
  CONSTRAINT `fk_Tool_has_Platform_Platform1`
    ...
    REFERENCES `tools_registry`.`platform` (`name` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE INDEX `fk_Comment_Comment_type1_idx` ON `tools_registry`.`comment` (`comment_type_uid` ASC) ;
...
-- -----------------------------------------------------
-- Table `tools_registry`.`table1`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `tools_registry`.`table1` ;

CREATE  TABLE IF NOT EXISTS `tools_registry`.`table1` (
)
ENGINE = InnoDB;
